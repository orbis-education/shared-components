...
const NavigationDropdown = ({
  ...
  const filteredDropdownItems = (navigationItem?.dropdownItems ?? []).filter(
    dropdownItem => isEmpty(dropdownItem.isPresent) || dropdownItem.isPresent === true
  );

  const dropdownClassnames = !isEmpty(navigationItem?.classes)
    ? `dropdown ${navigationItem.classes}`
    : "dropdown";

  return (
    <>
      <button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)} ref={dropdownRef}>
